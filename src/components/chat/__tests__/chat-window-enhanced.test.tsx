...
jest.mock("framer-motion", () => ({
  motion: {
    ...
    textarea: ({ children, ...props }: React.ComponentProps<"textarea">) => (
...
describe("ChatWindow Enhanced Features", () => {
  ...
  it("renders tool execution results when present", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("Successfully navigated to projects page")
      ).toBeInTheDocument();
  ...
  it("shows enhanced loading indicator during tool execution", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("Processing request...")
  ...
  it("handles multiple tool calls in a single response", async () => {
    ...
    fireEvent.change(input, {
      target: { value: "Show projects in dark mode" },
    });
    fireEvent.click(sendButton);
  ...
  it("handles API errors gracefully", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/I apologize, but I'm having trouble responding/)
      ).toBeInTheDocument();
    });
  });
