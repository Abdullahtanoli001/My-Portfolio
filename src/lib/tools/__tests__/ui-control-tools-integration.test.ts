...
describe("UI Control Tools Integration", () => {
  ...
  describe("Tool Registration", () => {
    ...
    it("should provide OpenAI function definitions", () => {
      ...
      expect(themeFunction?.description).toContain(
        "Switch between light and dark themes"
      );

      const downloadFunction = functions.find(
        (f) => f.name === "trigger_download"
      );
      expect(downloadFunction).toBeDefined();
      expect(downloadFunction?.description).toContain(
        "Trigger download of files"
      );

      const uiStateFunction = functions.find(
        (f) => f.name === "manage_ui_state"
      );
  ...
  describe("Performance and Statistics", () => {
    it("should track execution statistics for UI control tools", async () => {
      ...
      const downloadTool = toolRegistry.get(
        "trigger_download"
      ) as TriggerDownloadTool;
      const uiStateTool = toolRegistry.get(
        "manage_ui_state"
      ) as ManageUIStateTool;
